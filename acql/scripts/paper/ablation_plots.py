...
def plot_grouped_success_rate_metrics(df):
    ...
    fig.legend(handles, labels, fontsize='xx-large', loc='lower center', fancybox=True, shadow=True, ncol=3, bbox_to_anchor=(0.5, 0.01))

    plt.tight_layout(rect=[0, 0.05, 1, 1])
    fig.savefig("./ablation-plots-episode-sr.pdf")
    # plt.show()
...
def plot_grouped_reward_metrics(df):
    ...
    plt.tight_layout(rect=[0, 0.05, 1, 1])
    fig.savefig("./ablation-plots-episode-reward.pdf")
    # plt.show()


if __name__ == "__main__":

    lab_pc_df = pd.read_csv("./lab-pc-ablation-data.csv")
    phd_server_df = pd.read_csv("./phd-server-ablation-data.csv")
