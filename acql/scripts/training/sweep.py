...
def acql_train(run, task, seed, spec, margin=1.0, 
    ...
    return training_run(
        ...
        hyperparameters=task.acql_hps | { "episode_length": episode_length,
    ...
    for option_length, episode_length in zip(option_length_l, episode_length_l):
        for multiplier_num_sgd_steps in multiplier_num_sgd_steps_l:
            train_for_all(["SimpleMaze3D"], ["TwoSubgoals"], acql_train, "ACQL", seed_range=(0, max_seed),
                          ...
                          multiplier_num_sgd_steps=multiplier_num_sgd_steps)
            # train_for_all(["SimpleMaze3D"], ["Branching1"], acql_train, "ACQL", seed_range=(3, 5),
            #               option_length=option_length,
            #               episode_length=episode_length,
            #               multiplier_num_sgd_steps=multiplier_num_sgd_steps)
            ...
            #               episode_length=episode_length,
            #               multiplier_num_sgd_steps=multiplier_num_sgd_steps)
            # train_for_all(["SimpleMaze3D"], ["Until2"], acql_train, "ACQL", seed_range=(3, 5),
            #               option_length=option_length,
            #               episode_length=episode_length,
            #               multiplier_num_sgd_steps=multiplier_num_sgd_steps)
