...
- Action space:      3-dim, each element in [-1, 1], corresponding to target delta position for EEF

See _get_obs() and ArmEnvs._convert_action() for details.
"""

class ArmEEFPushEasy(ArmEnvs):
    def _get_xml_path(self):
        return os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'assets', "arm_eef_push_easy.xml")

    @property
    def action_size(self) -> int:
        return 3 # Override default (actuator count)
    ...
    def _set_environment_attributes(self):
        ...
        self.goal_indices = jnp.array([11, 12, 13])  # Cube position
    ...
    def _get_initial_goal(self, pipeline_state: base.State, rng):
        ...
        cube_goal_pos = jnp.array([0.1, 0.6, 0.03]) + jnp.array([self.goal_noise_scale, self.goal_noise_scale, 0]) * jax.random.uniform(subkey, [3], minval=-1)
    ...
    def reached_goal(self, obs: jax.Array, threshold: float = 0.1):
        ...
        goal_pos = obs[..., self.goal_indices]
        dist = jnp.linalg.norm(current_cube_pos - goal_pos)
