...
from acql.brax.envs.manipulation.ur5e_grasp import UR5eGrasp
from acql.brax.envs.manipulation.ur5e_reach import UR5eReach
from acql.brax.envs.manipulation.ur5e_push_easy import UR5ePushEasy
from acql.brax.envs.manipulation.ur5e_push_hard import UR5ePushHard
from acql.brax.envs.manipulation.ur5e_reach_shelf import UR5eReachShelf
from acql.brax.envs.manipulation.ur5e_reach_shelf_eef import UR5eReachShelfEEF
...
from acql.hierarchy.ur5e.load import load_ur5e_options, load_ur5e_eef_options
...
class UR5eReachTaskBase(UR5eTaskBase):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def _create_vars(self):
        self.wp_var = Var("wp", idx=0, dim=3)
        self.obs_var = Var("obs", idx=0, dim=self.env.state_dim, position=(6, 9))

    def _build_env(self, backend: str) -> GoalConditionedEnv:
        env = UR5eReach(backend=backend)
        return env

    @property
    def ppo_hps(self):
        return super().ppo_hps | { "episode_length": 100 }

    @property
    def sac_hps(self):
        return super().sac_hps | { "episode_length": 100 }

    @property
    def sac_her_hps(self):
        return super().sac_her_hps | { "episode_length": 100 }

    @property
    def ddpg_hps(self):
        return super().ddpg_hps | { "episode_length": 100 }

    @property
    def ddpg_her_hps(self):
        return super().ddpg_her_hps | { "episode_length": 100 }

    @property
    def crl_hps(self):
        return super().crl_hps | { "episode_length": 100, "multiplier_num_sgd_steps": 16 }

    @property
    def acddpg_hps(self):
        return super().acddpg_hps | { "episode_length": 100,
                                      "batch_size": 128,
...
class UR5eReachTrue(UR5eReachTaskBase):
...
class UR5eReachSingleSubgoal(SingleSubgoalMixin, UR5eReachTaskBase):
    def __init__(self, backend="mjx"):
        self.goal1_location = jnp.array([0.0, 0.5, 0.3])
        self.goal1_radius = 0.1

        super().__init__(None, 1000, backend=backend)


class UR5ePushEasyTaskBase(UR5eTaskBase):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def _build_env(self, backend: str) -> GoalConditionedEnv:
        env = UR5ePushEasy(backend=backend)
        return env

    @property
    def ppo_hps(self):
        return super().ppo_hps | { "episode_length": 150 }

    @property
    def sac_hps(self):
        return super().sac_hps | { "episode_length": 150 }

    @property
    def sac_her_hps(self):
        return super().sac_her_hps | { "total_env_steps": 20_000_000,
                                       "episode_length": 150,
                                       "batch_size": 128,
                                       "train_step_multiplier": 64 }
    ...
    @property
    def ddpg_hps(self):
        return super().ddpg_hps | { "episode_length": 150 }

    @property
    def ddpg_her_hps(self):
        return super().ddpg_her_hps | { "total_env_steps": 20_000_000,
                                        "episode_length": 150,
                                        "batch_size": 128,
                                        "train_step_multiplier": 64 }
    ...
    @property
    def crl_hps(self):
        return super().crl_hps | { "episode_length": 150, "multiplier_num_sgd_steps": 16 }

    @property
    def acddpg_hps(self):
        return super().acddpg_hps | { "total_env_steps": 4_000_000,
                                      "episode_length": 150,
                                      "batch_size": 128,
                                      "train_step_multiplier": 64 }


class UR5ePushEasyTrue(UR5ePushEasyTaskBase):
    def __init__(self, backend="mjx"):
        super().__init__(None, 1000, backend=backend)
...
class UR5ePushEasySingleSubgoal(SingleSubgoalMixin, UR5ePushEasyTaskBase):
    def __init__(self, backend="mjx"):
        self.goal1_location = jnp.array([0.1, 0.4, 0.03])
        self.goal1_radius = 0.1

        super().__init__(None, 1000, backend=backend)


class UR5ePushHardTaskBase(UR5eTaskBase):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def _build_env(self, backend: str) -> GoalConditionedEnv:
        env = UR5ePushHard(backend=backend)
        return env
    ...
    @property
    def acddpg_hps(self):
        return super().acddpg_hps | { "total_env_steps": 25_000_000,
                                      "episode_length": 150,
                                      "batch_size": 128,
                                      "train_step_multiplier": 64 }


class UR5ePushHardTrue(UR5ePushHardTaskBase):
    def __init__(self, backend="mjx"):
        super().__init__(None, 1000, backend=backend)
...
class UR5ePushHardSingleSubgoal(SingleSubgoalMixin, UR5ePushHardTaskBase):
    def __init__(self, backend="mjx"):
...
class UR5ePushHardTwoSubgoals(TwoSubgoalsMixin, UR5ePushHardTaskBase):
    def __init__(self, backend="mjx"):
        self.goal1_location = jnp.array([0.40, 0.25, 0.03])
        self.goal1_radius = 0.1
        self.goal2_location = jnp.array([0.10, 0.55, 0.03])
        self.goal2_radius = 0.1

        super().__init__(None, 1000, backend=backend)


class UR5ePushHardBranching1(Branching1Mixin, UR5ePushHardTaskBase):
    def __init__(self, backend="mjx"):
        self.goal1_location = jnp.array([0.40, 0.25, 0.03])
        self.goal1_radius = 0.1
        self.goal2_location = jnp.array([0.10, 0.55, 0.03])
