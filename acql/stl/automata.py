...
def get_func_from_spot_formula(form, n_bits):

    ap_vals = {}
    for i in range(n_bits):
        ap_vals[f"ap_{i}"] = 2**i

    def get_func_helper(root, children):
        children = list(children)

        k = root.kind()

        if k == spot.op_ff:
            return lambda bv: jnp.bool(False)
        elif k == spot.op_tt:
            return lambda bv: jnp.bool(True)
        elif k == spot.op_ap:
            return lambda bv: jnp.bool(bv & ap_vals[root.ap_name()])
        elif k == spot.op_Not:
            return lambda bv: jnp.logical_not(children[0](bv))
        elif k == spot.op_And:
            return lambda bv: jnp.logical_and(children[0](bv), children[1](bv))
        elif k == spot.op_Or:
            return lambda bv: jnp.logical_or(children[0](bv), children[1](bv))
        else:
            raise NotImplementedError(f"Formula {root} with kind = {k}")

    return fold_spot_formula(get_func_helper, form)
