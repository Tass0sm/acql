    ...
    def step(self, state: State, option_and_key: Tuple[jax.Array, PRNGKey]) -> State:
        ...
        if isinstance(self.options[0].termination_policy, FixedLengthTerminationPolicy):
            ...
            def for_body(unused, x):
                ...
                a_t, _ = jax.lax.switch(o_t, [o.inference for o in self.options], self.extra_adapter(s_t.observation), inf_key)
                s_t1 = self.env.step(s_t, a_t)
                s_t1.info["low_steps"] = s_t.info["low_steps"] + 1
            ...
            def while_cond(x):
                s_t, iters, _, _, key = x
                beta_t = jax.lax.switch(o_t, [o.termination for o in self.options], self.extra_adapter(s_t.observation), key)
