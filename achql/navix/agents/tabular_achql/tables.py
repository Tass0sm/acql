...
def make_option_inference_fn(
  ...
  def make_policy(
    ...
    def greedy_safe_option_policy(observation: types.Observation,
                                  option_key: PRNGKey) -> Tuple[types.Action, types.Extra]:
      qs = hdcq_networks.option_q_table.apply(option_q_params, observation)
      cqs = hdcq_networks.cost_q_table.apply(cost_q_params, observation)
