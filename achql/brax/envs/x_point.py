...
def make_maze(maze_layout_name, maze_size_scaling):
    if maze_layout_name == "fisac_maze":
        maze_layout = FISAC_MAZE
    else:
        raise ValueError(f"Unknown maze layout: {maze_layout_name}")

    xml_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'assets', "x_point.xml")

    possible_starts = find_starts(maze_layout, maze_size_scaling)
    possible_goals = find_goals(maze_layout, maze_size_scaling)

    # possible_goals = possible_goals.at[:].set(jnp.array([4., 12.]))
    possible_goals = possible_goals.at[:].set(jnp.array([12., 4.]))
...
class XPoint(PipelineEnv):
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        y_action = jnp.ones_like(action)
        full_action = jnp.concatenate((action, y_action), axis=-1)
