...
from achql.brax.agents.hsac import networks as soac_networks
from achql.brax.agents.hsac import losses as soac_losses
...
class TrajectoryUniformSamplingQueue(QueueBase[Sample], Generic[Sample]):
    ...
    def flatten_crl_fn(use_her, env, transition: h_types.HierarchicalTransition, sample_key: PRNGKey) -> h_types.HierarchicalTransition:
        if use_her:
...
def _init_training_state(
    ...
    policy_params = soac_network.hi_policy_network.init(key_policy)
    policy_optimizer_state = policy_optimizer.init(policy_params)
    q_params = soac_network.option_q_network.init(key_q)
...
def train(
    ...
    network_factory: types.NetworkFactory[soac_networks.SOACNetworks] = soac_networks.make_soac_networks,
    options=[],
    ...
    soac_network = network_factory(
        observation_size=obs_size,
        action_size=action_size,
        options=options,
        preprocess_observations_fn=normalize_fn,
    )
    make_policy = soac_networks.make_inference_fn(soac_network)
    ...
    dummy_transition = h_types.HierarchicalTransition(  # pytype: disable=wrong-arg-types  # jax-ndarray
        prev_option=0,
        termination=0,
        ...
        extras={
            ...
            "policy_extras": {
                "action_log_prob_given_option": 0.0,
                "option": 0,
                "option_log_prob": 0.0,
            },
        },
    ...
    alpha_loss, critic_loss, actor_loss = soac_losses.make_losses(
        soac_network=soac_network,
    ...
    def sgd_step(
        carry: Tuple[TrainingState, PRNGKey], transitions: h_types.HierarchicalTransition
    ...
    def get_experience(
        ...
        def f(carry, unused_t):
            env_state, option_state, current_key = carry
            current_key, next_key = jax.random.split(current_key)
            env_state, next_option_state, transition = hierarchical_acting.actor_step(
                env,
                env_state,
                option_state,
                policy,
            ...
            return (env_state, next_option_state, next_key), transition

        (env_state, option_state, _), data = jax.lax.scan(f, (env_state, option_state, key), (), length=unroll_length)
        ...
        return normalizer_params, env_state, option_state, buffer_state
    ...
    def training_step(
        ...
        buffer_state: ReplayBufferState,
        key: PRNGKey,
    ) -> Tuple[TrainingState, Union[envs.State, envs_v1.State], OptionState, ReplayBufferState, Metrics]:
        experience_key, training_key = jax.random.split(key)
        normalizer_params, env_state, option_state, buffer_state = get_experience(
            training_state.normalizer_params,
            training_state.policy_params,
            env_state,
            option_state,
        ...
        return training_state, env_state, option_state, buffer_state, metrics
    ...
    def prefill_replay_buffer(
        ...
        def f(carry, unused):
            del unused
            training_state, env_state, option_state, buffer_state, key = carry
            key, new_key = jax.random.split(key)
            new_normalizer_params, env_state, option_state, buffer_state = get_experience(
                training_state.normalizer_params,
                training_state.policy_params,
                env_state,
                option_state,
            ...
            return (new_training_state, env_state, option_state, buffer_state, new_key), ()

        return jax.lax.scan(
            f,
            (training_state, env_state, option_state, buffer_state, key),
            (),
    ...
    def training_epoch(
        ...
        buffer_state: ReplayBufferState,
        key: PRNGKey,
    ) -> Tuple[TrainingState, envs.State, OptionState, ReplayBufferState, Metrics]:
        def f(carry, unused_t):
            ts, es, os, bs, k = carry
            k, new_key, a_sgd_key = jax.random.split(k, 3)
            ts, es, os, bs, metrics = training_step(ts, es, os, bs, k)
            (ts, bs, a_sgd_key), _ = scan_additional_sgds(multiplier_num_sgd_steps - 1, ts, bs, a_sgd_key)
            return (ts, es, os, bs, new_key), metrics

        (training_state, env_state, option_state, buffer_state, key), metrics = jax.lax.scan(
            f,
            (training_state, env_state, option_state, buffer_state, key),
            (),
    ...
    def training_epoch_with_timing(
        ...
        (training_state, env_state, option_state, buffer_state, metrics) = training_epoch(
            training_state, env_state, option_state, buffer_state, key
        )
        ...
        return training_state, env_state, option_state, buffer_state, metrics  # pytype: disable=bad-return-type  # py311-upgrade
    ...
    # TODO: Figure out what CRL evaluator provides
    ...
    training_state, env_state, option_state, buffer_state, _ = prefill_replay_buffer(
        training_state, env_state, option_state, buffer_state, prefill_keys
    )
    ...
    for _ in range(num_evals_after_init):
        ...
        (training_state, env_state, option_state, buffer_state, training_metrics) = training_epoch_with_timing(
            training_state, env_state, option_state, buffer_state, epoch_keys
        )
