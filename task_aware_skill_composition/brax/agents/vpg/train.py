...
from task_aware_skill_composition.brax.agents.vpg import networks as vpg_networks
...
def train(
        ...
        network_factory: types.NetworkFactory[vpg_networks.VPGNetworks] = vpg_networks.make_vpg_networks,
    ...
    global_rng_key, local_rng_key = jax.random.split(rng_key)
    del rng_key
    # local_key = jax.random.fold_in(local_key, process_id)
    local_rng_key, env_rng_key, eval_rng_key = jax.random.split(local_rng_key, 3)
    ...
    env_rng_keys = jax.random.split(env_rng_key, num_envs // process_count)
    env_rng_keys = jnp.reshape(env_rng_keys, (local_devices_to_use, -1) + env_rng_keys.shape[1:])

    env_state = reset_fn(env_rng_keys)

    ## Make observation preprocessing function
    ## TODO: Investigate
    ...
    ## Make agent networks
    vpg_network = network_factory(
        env_state.obs.shape[-1],
        env.action_size,
        preprocess_observations_fn=normalize)
    make_policy = vpg_networks.make_inference_fn(vpg_network)
    ...
    def training_epoch(
            training_state: TrainingState, state: envs.State,
            key: PRNGKey
