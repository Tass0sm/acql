...
from task_aware_skill_composition.brax.agents.ddpg_lagrangian import losses as ddpg_lagrangian_losses
from task_aware_skill_composition.brax.agents.ddpg_lagrangian import networks as ddpg_lagrangian_networks
...
def _init_training_state(
  ...
  policy_params = ddpg_lagrangian_network.policy_network.init(key_policy)
  policy_optimizer_state = policy_optimizer.init(policy_params)
  q_params = ddpg_lagrangian_network.q_network.init(key_q)
  q_optimizer_state = q_optimizer.init(q_params)
  cost_q_params = ddpg_lagrangian_network.q_network.init(key_cost_q)
...
def train(
    ...
    logdir = './logs',
    network_factory: types.NetworkFactory[ddpg_lagrangian_networks.DDPGLagrangianNetworks] = ddpg_lagrangian_networks.make_ddpg_lagrangian_networks,
  ...
  ddpg_lagrangian_network = network_factory(
      observation_size=obs_size,
      action_size=action_size,
      preprocess_observations_fn=normalize_fn,)
  make_policy = ddpg_lagrangian_networks.make_inference_fn(ddpg_lagrangian_network)
  ...
  def sgd_step(
    ...
    new_target_cost_q_params = jax.tree_util.tree_map(
      lambda x, y: x * (1 - tau) + y * tau, training_state.target_cost_q_params,
      cost_q_params)
