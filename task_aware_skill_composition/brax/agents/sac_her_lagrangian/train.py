...
"""Constrained Soft Actor-Critic training.

See: https://arxiv.org/pdf/2002.08550
"""
...
from task_aware_skill_composition.brax.agents.sac_her_lagrangian import losses as sac_lagrangian_losses
...
class TrainingState:
    ...
    Notably: alpha, lambda, Q, S (cost critic), and pi
    """
...
def _init_training_state(
    ...
    policy_params = sac_lagrangian_network.policy_network.init(key_policy)
    policy_optimizer_state = policy_optimizer.init(policy_params)
    q_params = sac_lagrangian_network.q_network.init(key_q)
    q_optimizer_state = q_optimizer.init(q_params)
    cost_q_params = sac_lagrangian_network.cost_q_network.init(key_cost_q)
...
def train(
    ...
    network_factory: types.NetworkFactory[sac_lagrangian_networks.SACLagrangianNetworks] = sac_lagrangian_networks.make_sac_lagrangian_networks,
    ...
    sac_lagrangian_network = network_factory(
        observation_size=obs_size, action_size=action_size, preprocess_observations_fn=normalize_fn
    )
    make_policy = sac_lagrangian_networks.make_inference_fn(sac_lagrangian_network)

    alpha_optimizer = optax.adam(learning_rate=3e-4)
    lambda_optimizer = optax.adam(learning_rate=3e-4)
    ...
    def sgd_step(
        ...
        key, key_alpha, key_lambda, key_critic, key_cost_critic, key_actor = jax.random.split(key, 6)
        ...
        # clamp lambda
        # lambda_params = jnp.clip(lambda_params, 0.0, 10.0)

        # old lambda multiplier before update
        ...
        new_target_cost_q_params = jax.tree_util.tree_map(
            lambda x, y: x * (1 - tau) + y * tau, training_state.target_cost_q_params, cost_q_params
        )
