"""Wrappers to support Brax Semi-MDP training."""
...
def wrap(
  ...
  Args:
    ...
    option_repeat: how many repeated options to take per step
  ...
  Returns:
    ...
    wrapped.
  """
  env = EpisodeWrapper(env, episode_length, option_repeat)
...
class VmapWrapper(Wrapper):
  ...
  def step(self, state: State, option: jax.Array, key: PRNGKey) -> State:
    return jax.vmap(self.env.step)(state, option, key)


class EpisodeWrapper(Wrapper):
  """Maintains episode step count and sets done at episode end."""

  def __init__(self, env: Env, episode_length: int, option_repeat: int):
    super().__init__(env)
    self.episode_length = episode_length
    self.option_repeat = option_repeat

  def reset(self, rng: jax.Array) -> State:
    state = self.env.reset(rng)
    # state.info['low_steps'] = jp.zeros(rng.shape[:-1])
    state.info['high_steps'] = jp.zeros(rng.shape[:-1])
    state.info['truncation'] = jp.zeros(rng.shape[:-1])
    return state

  def step(self, state: State, option: jax.Array, key: PRNGKey) -> State:
    def f(state, _):
      nstate = self.env.step(state, option, key)
      return nstate, nstate.reward

    state, rewards = jax.lax.scan(f, state, (), self.action_repeat)
    state = state.replace(reward=jp.sum(rewards, axis=0))
    high_steps = state.info['high_steps'] + self.option_repeat
    ...
    state.info['high_steps'] = high_steps
...
class AutoResetWrapper(Wrapper):
  ...
  def step(self, state: State, option: jax.Array, key: PRNGKey) -> State:
    if 'high_steps' in state.info:
      high_steps = state.info['high_steps']
      high_steps = jp.where(state.done, jp.zeros_like(high_steps), steps)
      state.info.update(high_steps=high_steps)
    state = state.replace(done=jp.zeros_like(state.done))
    state = self.env.step(state, option, key)
